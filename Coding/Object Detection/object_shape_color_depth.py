...
try:
    while True:
        ...
        for contour in contours:
            ...
            # Calculate 3D position (center of mass) and orientation
            ...
            if M["m00"] != 0:
                ...
                if 0 <= cx < depth_roi.shape[1] and 0 <= cy < depth_roi.shape[0]:
                    # Get depth at center point
                    depth = depth_roi[cy, cx] * depth_scale
                    
                    # Convert 2D point to 3D camera coordinates
                    fx = 600  # Focal length x (replace with your camera's fx)
                    fy = 600  # Focal length y (replace with your camera's fy)
                    cx_cam = 320  # Principal point x (replace with your camera's cx)
                    cy_cam = 240  # Principal point y (replace with your camera's cy)
                    ...
                    # Calculate orientation (simplified - using PCA for dominant direction)
                    # This is a simplified approach - consider using more robust methods for production
                    if len(contour) >= 5:  # Need at least 5 points for PCA
                        # Convert contour to numpy array
                        cnt = contour.reshape(-1, 2)
                        mean = np.empty((0))
                        mean, eigenvectors = cv2.PCACompute(cnt.astype(np.float32), mean)
                        
                        # Get rotation matrix from principal components
                        # This gives us the orientation in 2D
                        angle = np.arctan2(eigenvectors[0,1], eigenvectors[0,0])
                        
                        # Create 3D rotation matrix (assuming rotation around Z-axis only)
                        # In a real scenario, you'd want to estimate full 3D orientation
                        rot_z = np.array([
                        ...
                        # Create 4x4 transformation matrix (camera to object)
                        T_cam_to_obj = np.eye(4)
                        T_cam_to_obj[:3, :3] = rot_z
                        T_cam_to_obj[:3, 3] = [x, y, z]
                        
                        # Store the transformation matrix with object info
                        object_info = {
                            'type': f"{color_name}_{shape_name}",
                            'color': color_name,
                            'shape': shape_name,
                            'position_cam': [x, y, z],  # In camera coordinates
                        ...
                        # Print or store the object info
                        print(f"Detected {object_info['type']} at position: {object_info['position_cam']}")
                        print(f"Transformation matrix (camera to object):\n{np.array2string(T_cam_to_obj, precision=3, suppress_small=True)}\n")
